...
```text
nixosConfigurations.<user>
└── users/<name>/default.nix    # Entrypoint (nixosSystem) + program imports
    ├── hosts/<name>/host.nix       # host-specific settings + feature imports
    └── hosts/<name>/hardware.nix   # disks, platform, initrd modules
```

Example for `dark1zin` on `midas`:

```nix
# modules/users/dark1zin/default.nix
flake.nixosConfigurations.dark1zin = inputs.nixpkgs.lib.nixosSystem {
  specialArgs = { inherit inputs self; };
  modules = [
    self.nixosModules.dark1zin        # imports home + programs + host modules
  ];
};
```
...
| File | Exports | Role |
|------|---------|------|
| `default.nix` | `nixosModules.<name>` & `nixosConfigurations.<name>` | Wires the flake output, Home Manager entry + host/program imports |
...
Programs and target hosts are independent modules. Swap target hosts or applications by commenting imports in `default.nix`.
...
**Target midas and configure git** — edit `modules/users/dark1zin/default.nix`:
...
```bash
nix flake check
nix build .#nixosConfigurations.<user>.config.system.build.toplevel
```
